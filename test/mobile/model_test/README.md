...
These test models will be executed in iOS simulator tests. Note that we only check if there's error in model execution, but don't check the correctness of model output.
...
NOTE: iOS test generates on-the-fly models.
...
You can also run these tests locally, please see the instruction in the ios folder. Remember to generate on-the-fly test models if you want to test it locally (but don't commit these models with _temp suffix).
```
...
You can also update all test models for iOS:
```
